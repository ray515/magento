...
class Mage_Page_Block_Html_Head extends Mage_Core_Block_Template
{
    ...
    /**
     * Add external js link and inject js to deliver 
     * with link for params and things like noConflict rules
     * 
     * 
     * 
     ...
     * @return Mage_Page_Block_Html_Head
     */
    public function addOutItem($outLink, $params=null){
    	$this->_data['items']['js_out/'.$outLink]=array(
    			'type'   =>'js_out',
    			'name'   => $outLink,
    			'params' => $params,
    			'if'     => $if,
     ...
     * ERIC: Added functionality to push js with external links i.e. google cdn for jQuery and the ability to inject js with method.
     *
     * @return string
     */
    public function getCssJsHtml()
    {
        ...
        foreach ($this->_data['items'] as $item) {
            ...
            switch ($item['type']) {
                case 'js_out':  // outside js links i.e. google api libs for jQuery... ;)
               		$lines[$if][$item['type']][$item['type']."/".$item['name']]['name']=$item['name'];
               		$lines[$if][$item['type']][$item['type']."/".$item['name']]['params']=$item['params'];
               		break;
               	case 'js':        // js/*.js
        ...
        foreach ($lines as $if => $items) {
            ...
            // outside js link and script injection to cover jQuery use.
            if (!empty($items['js_out'])) {
            	foreach($items['js_out'] as $out1){
            		$html .="<script type='text/javascript' src='".$out1['name']."'></script>\n";
            		if(!empty($out1['params'])){
            			$html .="<script type='text/javascript'>".$out1['params']."</script>\n";
            		}
            	}
            }
