...
class Searcher_Solr_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function searchPrice($sCol){
		...
		foreach($priceSetArr as $psa1=>$psa1a){
			...
			if($psa1 == $pFilt){
				$priceOut=$priceOut.'<li class="priceLI liTouch">'.$psa1.' </li>';
			}else{
				$priceOut=$priceOut.'<li class="priceLI liStart">'.$psa1.' </li>';//('.count($psa1a).')
			}
...
//		}
	...
	public function searchManu($sCol){
		$session = Mage::getSingleton('core/session', array('name'=>'frontend'));
		foreach ($sCol as $prod){
			$mid=$prod->getManufacturer();
			$mnm=$prod->getAttributeText('manufacturer');
			$manuInfo[$mid]=$mnm;
		}
